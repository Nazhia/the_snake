...
class Snake(GameObject):
    ...
    def position_corrector(self, position):
        """Проверяет позицию змейки."""
        new_pos = position
        if (position[0] > SCREEN_WIDTH):
            new_pos = (new_pos[0] - SCREEN_WIDTH, new_pos[1])
        if (position[0] < 0):
            new_pos = (new_pos[0] + SCREEN_WIDTH, new_pos[1])
        if (position[1] > SCREEN_HEIGHT):
            new_pos = (new_pos[0], new_pos[1] - SCREEN_HEIGHT)
        if (position[1] < 0):
            new_pos = (new_pos[0], new_pos[1] + SCREEN_HEIGHT)
        return new_pos
    ...
    def move(self):
        ...
        if (self.direction == RIGHT):
            ...
            new_head_pos = (head_pos[0] + GRID_SIZE, head_pos[1])
            new_head_pos = self.position_corrector(new_head_pos)
            ...
            head_pos = self.positions[0]
            new_head_pos = (head_pos[0], head_pos[1] + GRID_SIZE)
            new_head_pos = self.position_corrector(new_head_pos)
            self.last = self.positions[-1]
...
def main():
    ...
    while True:
        ...
        if (snake.length > 1):
            for pos_id in range(1, snake.length):
                if (snake.positions[0] == snake.positions[pos_id]):
                    snake.reset()
